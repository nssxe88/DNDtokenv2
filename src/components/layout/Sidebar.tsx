...
import { ImagePlus, CircleUser, Layers, Printer, Globe, X } from 'lucide-react';
...
export function Sidebar() {
  ...
  // Close sidebar on mobile when clicking outside (via overlay)
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-40 flex h-full w-72 flex-shrink-0 flex-col border-r border-slate-700 bg-slate-800 sm:w-80 lg:static lg:z-auto">
        {/* Mobile close button */}
        <div className="flex items-center justify-between border-b border-slate-700 px-3 py-2 lg:hidden">
          <span className="text-sm font-medium text-slate-200">Fantasy Token Printer</span>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <AdSlot
          ...
          width={300}
          height={250}
          className="hidden flex-shrink-0 border-t border-slate-700 p-2 sm:block"
        />
      </aside>
    </>
  );
